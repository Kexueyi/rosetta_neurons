...
def get_cvcl_clip_activation(cvcl_path, clip_path,  device, n = 5):
    ''' Obtain the Rosetta Neurons of the set of five discriminative models (ResNet50, MAE, DINO ResNet50, DINO ViTb16, CLIP ResNet50) 
    and a GAN using the n nearest neighbors. Returns dictionary of "best buddies" for the GAN and the five other models along with 
    activation statistics.'''
    
    table4, gan1_stats, cvcl_stats = loading.load_stats(cvcl_path, device)
    table5, gan2_stats, discr5_stats = loading.load_stats(clip_path, device)
    ...
    all_gan_matches = [list(perfect_matches4.keys()), list(perfect_matches5.keys())]
    all_discr_matches = [perfect_matches4, perfect_matches5]
    num_universal_matches = 0
    ...
    return universal_matches, [(gan1_stats, cvcl_stats), (gan2_stats, discr5_stats)]
